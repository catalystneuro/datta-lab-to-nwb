...
def reproduce_fig1d(nwbfile):
    n_frames = 361
    start = 3520
    position = pd.DataFrame(nwbfile.processing['behavior']['Position']['SpatialSeries'].data,
                            columns=['x', 'y', 'height'])
    angle = pd.DataFrame(nwbfile.processing['behavior']['CompassDirection']['SpatialSeries'].data,
                         columns=['angle'])
    syllables = pd.DataFrame(nwbfile.processing['behavior']['SyllableTimeSeries'].time_series['syllable'].data,
                             columns=['syllable'])
    vel_height = (
            position['height']
