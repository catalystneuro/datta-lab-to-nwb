...
class MarkowitzGillisNature2023BehaviorInterface(BaseDataInterface):
    ...
    def get_metadata(self, session_uuid: str) -> dict:
        ...
        session_df = pd.read_parquet(self.source_data['filename'],
                                     columns=self.source_data['columns'],
                                     filters=[('uuid', '==', session_uuid)])
        ...
        session_name = (
                set(session_df.session_name[session_df.session_name.notnull()]) |
                set(session_df.SessionName[session_df.SessionName.notnull()])
                        )
